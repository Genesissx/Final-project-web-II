...
export default function ServiceCard({ service }) {
  const ratingLabel = formatRating(service?.rating);

  return (
    <Link to={`/detail/${service.id}`} className="employee-card">
      <div className="employee-card-avatar">
        <img src={service.avatar || DEFAULT_AVATAR} alt={service.name || "Prestataire"} />
      </div>
      <div className="employee-card-content">
        <div className="employee-card-header">
          <h3>{service.name}</h3>
          <span>{getServiceCategory(service) || "—"}</span>
        </div>
        ...
        <div className="employee-card-body">
          ...
          <p><strong>Note :</strong> {ratingLabel} / 10</p>
        </div>
      </div>
    </Link>
  );
